...
func (rm *resourceManager) sdkFind(
	...
	var resp *svcsdk.DescribeEgressOnlyInternetGatewaysOutput
	resp, err = rm.sdkapi.DescribeEgressOnlyInternetGateways(ctx, input)
	rm.metrics.RecordAPICall("READ_MANY", "DescribeEgressOnlyInternetGateways", err)
	if err != nil {
...
func (rm *resourceManager) sdkCreate(
	...
	var resp *svcsdk.CreateEgressOnlyInternetGatewayOutput
	_ = resp
	resp, err = rm.sdkapi.CreateEgressOnlyInternetGateway(ctx, input)
	rm.metrics.RecordAPICall("CREATE", "CreateEgressOnlyInternetGateway", err)
	if err != nil {
		return nil, err
	}
	// Merge in the information we read from the API call above to the copy of
	...
	if resp.EgressOnlyInternetGateway.Attachments != nil {
		f0 := []*svcapitypes.InternetGatewayAttachment{}
		for _, f0iter := range resp.EgressOnlyInternetGateway.Attachments {
			f0elem := &svcapitypes.InternetGatewayAttachment{}
			if f0iter.State != "" {
				f0elem.State = aws.String(string(f0iter.State))
			}
	...
	if resp.EgressOnlyInternetGateway.Tags != nil {
		f2 := []*svcapitypes.Tag{}
		for _, f2iter := range resp.EgressOnlyInternetGateway.Tags {
			f2elem := &svcapitypes.Tag{}
			if f2iter.Key != nil {
				f2elem.Key = f2iter.Key
			}
...
func (rm *resourceManager) sdkUpdate(
	...
	return rm.customUpdateEgressOnlyInternetGateway(ctx, desired, latest, delta)
}
...
func (rm *resourceManager) sdkDelete(
	...
	var resp *svcsdk.DeleteEgressOnlyInternetGatewayOutput
	_ = resp
	resp, err = rm.sdkapi.DeleteEgressOnlyInternetGateway(ctx, input)
	rm.metrics.RecordAPICall("DELETE", "DeleteEgressOnlyInternetGateway", err)
	return nil, err
}
