...
func (rm *resourceManager) lateInitializeFromReadOneOutput(
	...
	if observedKo.Spec.ExcessCapacityTerminationPolicy != nil && latestKo.Spec.ExcessCapacityTerminationPolicy == nil {
		latestKo.Spec.ExcessCapacityTerminationPolicy = observedKo.Spec.ExcessCapacityTerminationPolicy
	}
	if observedKo.Spec.OnDemandOptions != nil && latestKo.Spec.OnDemandOptions == nil {
		latestKo.Spec.OnDemandOptions = observedKo.Spec.OnDemandOptions
	}
	if observedKo.Spec.ReplaceUnhealthyInstances != nil && latestKo.Spec.ReplaceUnhealthyInstances == nil {
		latestKo.Spec.ReplaceUnhealthyInstances = observedKo.Spec.ReplaceUnhealthyInstances
	}
	if observedKo.Spec.SpotOptions != nil && latestKo.Spec.SpotOptions == nil {
		latestKo.Spec.SpotOptions = observedKo.Spec.SpotOptions
	}
	if observedKo.Spec.TargetCapacitySpecification != nil && latestKo.Spec.TargetCapacitySpecification != nil {
		if observedKo.Spec.TargetCapacitySpecification.OnDemandTargetCapacity != nil && latestKo.Spec.TargetCapacitySpecification.OnDemandTargetCapacity == nil {
			latestKo.Spec.TargetCapacitySpecification.OnDemandTargetCapacity = observedKo.Spec.TargetCapacitySpecification.OnDemandTargetCapacity
		}
	}
	if observedKo.Spec.TargetCapacitySpecification != nil && latestKo.Spec.TargetCapacitySpecification != nil {
		if observedKo.Spec.TargetCapacitySpecification.SpotTargetCapacity != nil && latestKo.Spec.TargetCapacitySpecification.SpotTargetCapacity == nil {
			latestKo.Spec.TargetCapacitySpecification.SpotTargetCapacity = observedKo.Spec.TargetCapacitySpecification.SpotTargetCapacity
		}
	}
	if observedKo.Spec.TerminateInstancesWithExpiration != nil && latestKo.Spec.TerminateInstancesWithExpiration == nil {
		latestKo.Spec.TerminateInstancesWithExpiration = observedKo.Spec.TerminateInstancesWithExpiration
	}
...
func (rm *resourceManager) IsSynced(ctx context.Context, res acktypes.AWSResource) (bool, error) {
	...
	if r.ko.Status.ActivityStatus == nil {
		return false, nil
	}
	activityStatusCandidates := []string{"fulfilled"}
	if !ackutil.InStrings(*r.ko.Status.ActivityStatus, activityStatusCandidates) {
		return false, nil
	}
	if r.ko.Status.FleetState == nil {
		return false, nil
	}
	fleetStateCandidates := []string{"active"}
	if !ackutil.InStrings(*r.ko.Status.FleetState, fleetStateCandidates) {
		return false, nil
	}
