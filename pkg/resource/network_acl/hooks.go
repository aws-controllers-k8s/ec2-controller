...
func (rm *resourceManager) syncAssociation(
	...
	deleterr := rm.deleteOldAssociations(ctx, desired, latest, to_Delete)
	if deleterr != nil {
		return deleterr
	}
	return nil
...
func (rm *resourceManager) syncEntries(
	...
	if len(desired.ko.Spec.Entries) != len(uniq_entries) {
